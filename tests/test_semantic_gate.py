...
class SemanticGateTests(unittest.TestCase):
    ...
    def test_full_plan_converges_research_into_semantic_gate(self):
        tasks={t.id:t for t in build_plan('full',research=True,citation_verify=True)}
        self.assertIn('CITATION_REVIEW_READY',tasks['SEMANTIC_AUDIT'].requires);self.assertIn('SEMANTIC_GATE_PASS',tasks['OFFICE_COMPOSE'].requires)

    def test_heading_label_is_not_a_claim(self):
        td=tempfile.TemporaryDirectory();self.addCleanup(td.cleanup);p=Path(td.name)/'heading.md'
        p.write_text('# 第2章 分析\n## 2.3 有限元结果\n本节介绍分析方法。\n',encoding='utf-8');ast=parse_markdown(p)
        self.assertFalse(any(c['text'] in {'2.3 有限元结果','有限元结果'} for c in ast['claims']))

    def test_reviewer_returns_major_revision_not_reject_for_fixable_missing_evidence(self):
        result=audit(self._ast(),strict=True);self.assertEqual(result['review_decision'],'MAJOR_REVISION')
    ...
    def test_external_citation_major_revision_is_folded_into_semantic_review(self):
        citation_review={'review_decision':'MAJOR_REVISION','findings':[{'severity':'major','code':'CITATION_ENTITY_MISMATCH','reference':1,'required_action':'REPLACE_OR_CORRECT_REFERENCE'}]}
        result=audit(self._ast(),citation_review=citation_review,strict=True)
        self.assertEqual(result['review_decision'],'MAJOR_REVISION');self.assertTrue(any(x['owner']=='reference_team' for x in result['findings']))
