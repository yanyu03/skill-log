...
class SemanticGateTests(unittest.TestCase):
    ...
    def test_claim_provenance_extraction(self):
        ast = self._ast()
        kinds = {c['claim_type'] for c in ast['claims']}
        self.assertTrue({'measured', 'simulated', 'calculated', 'literature', 'design_target'} <= kinds)
        self.assertGreater(ast['metadata']['semantic_claim_count'], 0)
    ...
    def test_strict_blocks_unbacked_measured_and_simulated_claims(self):
        ...
        self.assertIn('MEASURED_CLAIM_WITHOUT_EVIDENCE', codes)
        self.assertIn('SIMULATION_CLAIM_WITHOUT_EVIDENCE', codes)

    def test_verified_evidence_allows_delivery(self):
        ast = self._ast()
        artifacts = []
        for claim in ast['claims']:
            if claim['claim_type'] == 'measured':
                artifacts.append({'id': 'exp-1', 'kind': 'experiment', 'claim_ids': [claim['id']], 'verified': True})
            elif claim['claim_type'] == 'simulated':
                artifacts.append({'id': 'sim-1', 'kind': 'solver_output', 'claim_ids': [claim['id']], 'verified': True})
        result = audit(ast, {'artifacts': artifacts}, strict=True)
