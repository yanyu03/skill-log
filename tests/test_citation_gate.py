...
class CitationGateTests(unittest.TestCase):
    ...
    def test_legacy_aggregate_provider_report_is_major_revision_not_reject(self):
        report={'schema':'host-citation-verify/v1','input_count':10,'verified_count':0,'invalid_count':3,'pending_count':7,'findings':[{'reference':14,'status':'INVALID_ENTITY_MISMATCH'},{'reference':17,'status':'INVALID_ENTITY_MISMATCH'},{'reference':19,'status':'INVALID_ENTITY_MISMATCH'}]}
        r=review_external(report,min_verified_english=5);self.assertEqual(r['status'],'PASS_W');self.assertEqual(r['review_decision'],'MAJOR_REVISION');self.assertEqual(r['scheduler_recommendation']['produce'],['CITATION_REVIEW_READY']);self.assertTrue(any(x['code']=='ENGLISH_VERIFIED_COUNT_UNAVAILABLE' for x in r['findings']))
